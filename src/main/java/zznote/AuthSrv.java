...
public class AuthSrv {
	...
	public static Pare<String, QDate> doAuth_ProduceToken(long nid, String additionalyMsgData) {
...
//		Integer fromId = msgCrt.msg_from_id();
		...
		if (hoursTimeout == null) {
			...
			if (X.empty(foundAllAliases)) {
				...
				Optional<WebUsr> first = foundAllAliases.stream().filter(u -> X.equals(u.net, net) && X.equals(u.nid, nid)).findFirst();
				if (first.isPresent()) {
					//ok found
				} else {
...
//		doAuth0(outRslt);
	...
	public static String produceTokenAndBuildFreshMsgWithLinkToken(NT nt, String additionalyMsgData, Long fromId) {
		Pare<String, QDate> auth = AuthSrv.doAuth_ProduceToken(fromId, additionalyMsgData);
		String msg = auth.key() + STR.NL + "http://" + nt.shortPfx() + fromId + "." + AppZzn.getAppSiteName(null) + "?t=" + auth.key() + STR.NL + "Токен действителен до " + auth.val().f(FDate.YYYY_DB_ISO_STANDART);
		return msg;
	}
}
