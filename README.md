...
#### Manual - [MANUAL](http://zznote.ru/_manual)

Платформа позволяет:

- создавать сайты, как набор простых заметок

- хранить и просматривать файлы, фото, видео и музыку как заметки

- писать различные скрипты на Phyton, Bash, Groovy, Java, Sql и выполнять программы по расписанию

- интегрироваться с различными системами посредством REST API и Kafka

- используя @{placeholder} читать, вызывать, вставлять различные вызовы и интеграции

- использование URL query для управления контекстом исполнения заметки 

- использоваться как rest server, файловый сервер, key/value хранилище

- использоваться как среда для создания сложных модулей и прототипов, авто- и интеграционных тестов ( mini NiFi )

- использоваться как java-фрэймворк для создания кастомных страниц и других сервисов или приложений
...
- пользователь отправляет боту '+' и получает токен приложения от тг-бота. Пользователь получает свой домен типа **t123.domain.com**
